describe "MixingpanelProperties", ->
  ...
  describe "check if search engine", ->
    it "should get google", ->
      mpp = new MixingpanelProperties("http://www.google.es?meh#wat#wat")
      expect(mpp.engine).toBe("google")

    it "should get yahoo", ->
      mpp = new MixingpanelProperties("http://www.yahoo.com?meh#wat#wat")
      expect(mpp.engine).toBe("yahoo")

    it "should get bing", ->
      mpp = new MixingpanelProperties("http://www.bing.com?meh#wat#wat")
      expect(mpp.engine).toBe("bing")

    it "should ignore a regular website", ->
      mpp = new MixingpanelProperties("http://meh.kelisto.es?q=meh#wat#wat")
      expect(mpp.engine).toBeNull()
      expect(mpp.search_terms).toEqual([])

    it "should ignore google plus", ->
      mpp = new MixingpanelProperties("http://plus.google.com?q=meh#wat#wat")
      expect(mpp.engine).toBeNull()
      expect(mpp.search_terms).toEqual([])

    it "should get the keywords for a google referrer", ->
      mpp = new MixingpanelProperties("http://www.google.com.ar/url?sa=t&rct=j&q=compra%20de%20vehiculos%20en%20el%20extranjero%20por%20internet&source=web&cd=4&ved=0CFMQFjAD&url=http%3A%2F%2Fwww.kelisto.es")
    ...
    it "should get the keywords for a yahoo referrer", ->
      mpp = new MixingpanelProperties("http://www.yahoo.com/?p=special+search+term&")
      expect(mpp.search_terms).toEqual([ 'special', 'search', 'term' ])

    it "should get the keywords for a bing referrer", ->
      mpp = new MixingpanelProperties("http://www.bing.com/?q=special+search+term")
      expect(mpp.search_terms).toEqual([ 'special', 'search', 'term' ])

    it "should have empty keywords for an unknown referrer", ->
      mpp = new MixingpanelProperties("http://www.example.com/?q=special+search+term")
      expect(mpp.search_terms).toEqual([])

    it "should have empty keywords with empty parameters", ->
      mpp = new MixingpanelProperties("http://www.google.com/?q=")
      expect(mpp.search_terms).toEqual([''])

    it "should have empty keywords with no parameters", ->
      mpp = new MixingpanelProperties("https://www.google.com/")
      expect(mpp.search_terms).toEqual([])
  ...
  describe "check if internal", ->
    it "should return True if kelisto.es domain", ->
      mpp = new MixingpanelProperties("http://meh.kelisto.es?meh#wat#wat")
      expect(mpp.isInternal()).toBe(true)

    it "should return False if not kelisto.es domain", ->
      mpp = new MixingpanelProperties("http://meh.helisto.es?kelisto.es#wat#wat")
  ...
  describe "check if social", ->
    ...
    it "should return True if twitter", ->
      mpp = new MixingpanelProperties("http://meh.twitter.com?meh#wat#wat")
      expect(mpp.isSocial()).toBe(true)

    it "should return True if t.co", ->
      mpp = new MixingpanelProperties("http://t.co?meh#wat#wat")
      expect(mpp.isSocial()).toBe(true)

    it "should return True if facebook", ->
      mpp = new MixingpanelProperties("http://meh.facebook.com?meh#wat#wat")
      expect(mpp.isSocial()).toBe(true)

    it "should return True if G+", ->
      mpp = new MixingpanelProperties("http://plus.google.com?meh#wat#wat")
      expect(mpp.isSocial()).toBe(true)

    it "should return False if regular google", ->
      mpp = new MixingpanelProperties("http://meh.google.com?meh#wat#wat")
      expect(mpp.isSocial()).toBe(false)
